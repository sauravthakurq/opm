package iad1tya.echo.music.data.repository
...
import iad1tya.echo.kotlinytmusicscraper.extension.verifyYouTubePlaylistId
import iad1tya.echo.kotlinytmusicscraper.models.AccountInfo
import iad1tya.echo.kotlinytmusicscraper.models.MediaType
import iad1tya.echo.kotlinytmusicscraper.models.MusicShelfRenderer
import iad1tya.echo.kotlinytmusicscraper.models.SearchSuggestions
...
import iad1tya.echo.kotlinytmusicscraper.models.response.LikeStatus
import iad1tya.echo.kotlinytmusicscraper.models.response.PlayerResponse
import iad1tya.echo.kotlinytmusicscraper.models.response.SearchResponse
import iad1tya.echo.kotlinytmusicscraper.models.echo.FdroidResponse
...
import iad1tya.echo.kotlinytmusicscraper.models.youtube.YouTubeInitialPage
import iad1tya.echo.kotlinytmusicscraper.pages.BrowseResult
import iad1tya.echo.kotlinytmusicscraper.pages.NextPage
import iad1tya.echo.kotlinytmusicscraper.pages.PlaylistPage
import iad1tya.echo.kotlinytmusicscraper.pages.SearchPage
import iad1tya.echo.kotlinytmusicscraper.parser.getPlaylistContinuation
import iad1tya.echo.kotlinytmusicscraper.parser.getPlaylistRadioEndpoint
import iad1tya.echo.kotlinytmusicscraper.parser.getPlaylistShuffleEndpoint
...
import iad1tya.echo.music.data.db.entities.PodcastWithEpisodes
...
import iad1tya.echo.music.data.model.browse.album.AlbumBrowse
import iad1tya.echo.music.data.model.browse.album.Track
import iad1tya.echo.music.data.model.browse.artist.ArtistBrowse
import iad1tya.echo.music.data.model.browse.playlist.Author
import iad1tya.echo.music.data.model.browse.playlist.PlaylistBrowse
import iad1tya.echo.music.data.model.explore.mood.Genre
import iad1tya.echo.music.data.model.explore.mood.Mood
import iad1tya.echo.music.data.model.explore.mood.MoodsMoment
import iad1tya.echo.music.data.model.explore.mood.genre.GenreObject
import iad1tya.echo.music.data.model.explore.mood.moodmoments.MoodsMomentObject
...
import iad1tya.echo.music.data.model.podcast.PodcastBrowse
...
import iad1tya.echo.music.data.parser.parseAlbumData
import iad1tya.echo.music.data.parser.parseArtistData
import iad1tya.echo.music.data.parser.parseChart
import iad1tya.echo.music.data.parser.parseGenreObject
import iad1tya.echo.music.data.parser.parseLibraryPlaylist
import iad1tya.echo.music.data.parser.parseMixedContent
import iad1tya.echo.music.data.parser.parseMoodsMomentObject
import iad1tya.echo.music.data.parser.parseNewRelease
import iad1tya.echo.music.data.parser.parseNextLibraryPlaylist
import iad1tya.echo.music.data.parser.parsePlaylistData
import iad1tya.echo.music.data.parser.parsePodcast
import iad1tya.echo.music.data.parser.parsePodcastContinueData
import iad1tya.echo.music.data.parser.parsePodcastData
import iad1tya.echo.music.data.parser.parseSetVideoId
import iad1tya.echo.music.data.parser.search.parseSearchAlbum
import iad1tya.echo.music.data.parser.search.parseSearchArtist
import iad1tya.echo.music.data.parser.search.parseSearchPlaylist
import iad1tya.echo.music.data.parser.search.parseSearchSong
import iad1tya.echo.music.data.parser.search.parseSearchVideo
import iad1tya.echo.music.data.parser.toListThumbnail
...
import iad1tya.echo.music.extension.toListTrack
import iad1tya.echo.music.extension.toLyrics
import iad1tya.echo.music.extension.toPlainLrcString
import iad1tya.echo.music.extension.toSongItemForDownload
import iad1tya.echo.music.extension.toSyncedLrcString
...
import aditya.echo.aiservice.AIHost
                    ...
                    dataStoreManager.aiProvider.collectLatest { provider ->
                        aiClient.host =
                            when (provider) {
                                ...
                                DataStoreManager.AI_PROVIDER_OPENROUTER -> AIHost.OPENROUTER
...
//                            }
    ...
    fun getYouTubeCaption(videoId: String): Flow<Resource<Pair<Lyrics, Lyrics?>>> =
        flow {
            runCatching {
                ...
                Log.d("Lyrics", "Getting YouTube caption for $videoId with language $preferLang")
                
                // Try with preferred language first
                youTube
                    .getYouTubeCaption(videoId, preferLang)
                    .onSuccess { lyrics ->
                        Log.d("Lyrics", "YouTube caption success: first=${lyrics.first != null}, second=${lyrics.second != null}")
                        if (lyrics.first != null) {
                            val convertedFirst = lyrics.first.toLyrics()
                            val convertedSecond = lyrics.second?.toLyrics()
                            Log.d("Lyrics", "Converted lyrics: first lines=${convertedFirst.lines?.size ?: 0}, second lines=${convertedSecond?.lines?.size ?: 0}")
                            emit(
                                Resource.Success<Pair<Lyrics, Lyrics?>>(
                                    Pair(convertedFirst, convertedSecond),
                                ),
                            )
                        } else {
                            Log.w("Lyrics", "YouTube caption first is null")
                            emit(Resource.Error<Pair<Lyrics, Lyrics?>>("No captions available"))
                        }
                    }.onFailure { e ->
                        Log.d("Lyrics", "YouTube caption error with $preferLang: ${e.message}")
                        
                        // Try with English as fallback
                        if (preferLang != "en") {
                            Log.d("Lyrics", "Trying YouTube caption with English fallback")
                            youTube
                                .getYouTubeCaption(videoId, "en")
                                .onSuccess { lyrics ->
                                    Log.d("Lyrics", "YouTube caption success with English: first=${lyrics.first != null}, second=${lyrics.second != null}")
                                    if (lyrics.first != null) {
                                        val convertedFirst = lyrics.first.toLyrics()
                                        val convertedSecond = lyrics.second?.toLyrics()
                                        Log.d("Lyrics", "Converted lyrics with English: first lines=${convertedFirst.lines?.size ?: 0}, second lines=${convertedSecond?.lines?.size ?: 0}")
                                        emit(
                                            Resource.Success<Pair<Lyrics, Lyrics?>>(
                                                Pair(convertedFirst, convertedSecond),
                                            ),
                                        ...
                                        Log.w("Lyrics", "YouTube caption first is null with English")
                                        emit(Resource.Error<Pair<Lyrics, Lyrics?>>("No captions available"))
                                    }
                                }.onFailure { e2 ->
                                    Log.d("Lyrics", "YouTube caption error with English: ${e2.message}")
                                    e2.printStackTrace()
                                    emit(Resource.Error<Pair<Lyrics, Lyrics?>>("No captions available: ${e2.message}"))
                                }
                        } else {
                            e.printStackTrace()
                            emit(Resource.Error<Pair<Lyrics, Lyrics?>>("No captions available: ${e.message}"))
                        }
                    }
            }
        }.flowOn(Dispatchers.IO)
    ...
    fun getLrclibLyrics(videoId: String): Flow<Resource<Lyrics>> =
        flow {
            runCatching {
                val watchEndpoint = WatchEndpoint(videoId = videoId)
                youTube
                    .next(watchEndpoint)
                    .onSuccess { nextPage ->
                        nextPage.lyricsEndpoint?.let { endpoint ->
                            youTube
                                .lyrics(endpoint)
                                .onSuccess { lyricsText ->
                                    if (!lyricsText.isNullOrEmpty()) {
                                        // Parse the lyrics text into Lyrics object
                                        // Try to parse LRC format first (with timestamps)
                                        val lines = if (lyricsText.contains("[") && lyricsText.contains("]")) {
                                            // LRC format with timestamps
                                            lyricsText.split("\n").mapNotNull { line ->
                                                val trimmedLine = line.trim()
                                                if (trimmedLine.isEmpty()) return@mapNotNull null

                                                // Try to parse LRC timestamp format [mm:ss.xx]
                                                ...
                                                if (match != null) {
                                                    ...
                                                    val startTimeMs = (minutes * 60 + seconds) * 1000 + centiseconds * 10

                                                    val words = trimmedLine.substring(match.range.last + 1).trim()
                                                    if (words.isNotEmpty()) {
                                                        Line(
                                                            startTimeMs = startTimeMs.toString(),
                                                            endTimeMs = (startTimeMs + 2000).toString(), // 2 seconds duration
                                                            words = words
                                                        )
                                                    } else null
                                                } else {
                                                    // No timestamp, treat as unsynced
                                                    Line(
                                                        startTimeMs = "0",
                                                        endTimeMs = "0",
                                                        words = trimmedLine
                                        ...
                                        val syncType = if (lines.any { it.startTimeMs != "0" }) "LINE_SYNCED" else "unsynced"
                                        val lyrics = Lyrics(
                                            lines = lines,
                                            syncType = syncType,
                                            error = false
                                        ...
                                        emit(Resource.Error("No lyrics found"))
                                    }
                                }
                                .onFailure { e ->
                                    emit(Resource.Error("Failed to fetch lyrics: ${e.message}"))
                                }
                        } ?: emit(Resource.Error("No lyrics endpoint found"))
                    }
                    .onFailure { e ->
                        emit(Resource.Error("Failed to get next page: ${e.message}"))
                    }
            }
        }.flowOn(Dispatchers.IO)

    // TODO: Implement Spotify playlist import functionality
    // This is a placeholder for future implementation
    fun getSpotifyPlaylist(playlistId: String): Flow<Resource<SpotifyPlaylistResponse>> =
        flow<Resource<SpotifyPlaylistResponse>> {
            try {
                // First, get a client token for public access
                val clientTokenResult = spotify.getClientToken()
                if (clientTokenResult.isFailure) {
                    emit(Resource.Error("Failed to get Spotify client token"))
                    return@flow
                }
                
                val clientToken = clientTokenResult.getOrThrow().grantedToken.token
                
                // Try to fetch as public playlist first (no user authentication required)
                spotify.getPublicPlaylist(playlistId, clientToken).onSuccess { playlist ->
                    emit(Resource.Success(playlist))
                }.onFailure { exception ->
                    // If public access fails, try with user authentication
                    val spdc = dataStoreManager.spdc.first()
                    val personalToken = dataStoreManager.spotifyPersonalToken.first()

                    if (spdc.isEmpty() || personalToken.isEmpty()) {
                        emit(Resource.Error("Playlist is private. Please login to Spotify to import private playlists."))
                        return@flow
                    }

                    // Check if personal token is expired and refresh if needed
                    ...
                    // Fetch playlist with user authentication
                    spotify.getPlaylist(playlistId, finalPersonalToken, clientToken).onSuccess { playlist ->
                        emit(Resource.Success(playlist))
                    }.onFailure { authException ->
                        emit(Resource.Error("Failed to fetch playlist: ${authException.message}"))
                    }
                }
            } catch (e: Exception) {
                emit(Resource.Error("Error importing playlist: ${e.message}"))
            }
        }.flowOn(Dispatchers.IO)
