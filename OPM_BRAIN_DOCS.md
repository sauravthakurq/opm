...
**OPM Brain** is an intelligent, on-device contextual music engine integrated directly into OPM. Designed to maintain the momentum and vibe of your current listening session, it continually analyzes your queue and proactively injects seamless, personalized track recommendations. 
...
3. **Curated "OPM Brain Recommends" Playlists**
   On your Home Screen, a dedicated "OPM Brain Recommends" section offers dynamically generated playlists containing up to 50 tracks each. These are curated exclusively from your listening momentum and the highest-rated songs in your local vault.

4. **100% On-Device & Private**
   OPM Brain builds its neural persona locally on your device. It maps your genre affinities, artist scores, and interactions without sending your data to external servers. It is strictly privacy-first.

---

## How It Works Under the Hood

At its core, OPM Brain is built around the `OpmBrainEngine`, which listens to the Media3 `PlayerConnection` state flow. It operates via a multi-stage pipeline whenever it detects a track transition:
...
* **Anchor (The Current Track)**: It queries the backend API's "Next" endpoint (`YouTube.next()`) using the currently playing track's ID to find immediate sonic neighbors.
...
The remaining candidates are passed into the `neuroEngine.rank()` function. This function scores each track against your personal `OpmBrainPersona`—a locally-stored profile built from your historical genre affinities, artist scores, and total interaction counts. 

### 3. Queue Injection
The top 3 highest-ranked tracks are selected to form the "Runway." The engine waits for a brief 1.5-second buffer (to avoid interrupting the Media3 player transition state) and seamlessly injects these items into the `Player` queue immediately after the current playing index. These items are tagged with the `QueueItemSource.OPM_BRAIN` source, ensuring the UI can badge them appropriately.
...
* **When Disabled**: Any previously injected "OPM Brain" tracks are instantly removed from your current queue, and the background tracking pauses entirely.
